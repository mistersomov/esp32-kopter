...
public:
    ...
    Device *add_device(const std::string &name, AdcMode mode, const std::vector<adc_channel_t> &channels);
...
private:
    ...
    void add_device_continuous(const std::vector<adc_channel_t> &channels);
    void add_device_one_shot(const std::vector<adc_channel_t> &channels);
    void start_polling();
    void stop_polling();
