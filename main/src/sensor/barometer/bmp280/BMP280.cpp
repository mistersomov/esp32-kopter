...
static constexpr uint8_t TEMP_UPPER_BYTE = 0xFA;
static constexpr uint8_t PRESSURE_UPPER_BYTE = 0xF7;
static constexpr uint8_t CALIB_UPPER_BYTE = 0x88;
static constexpr uint8_t TEMP_BYTES = 3;
static constexpr uint8_t PRESSURE_BYTES = 3;
static constexpr uint8_t CALIB_BYTES = 24;
...
public:
    ...
    float read_temperature()
    {
        ...
        int32_t raw =
            ((static_cast<int32_t>(result[0]) << 16) | (static_cast<int32_t>(result[1]) << 8) | result[2]) >> 4;

        return m_mapper->map_temperature(raw, m_calib);
    }
    ...
    float read_pressure()
    {
        ...
        return m_mapper->map_pressure(raw, m_calib);
    }
...
private:
    ...
    BMP280Calibration m_calib;
};

BMP280::BMP280(uint8_t address) : IBarometer(), m_impl{std::make_unique<Impl>(address)}
{
}
...
{
    return m_impl->read_temperature();
}

float BMP280::read_pressure()
{
    return m_impl->read_pressure();
}

float BMP280::read_altitude()
{
    return m_impl->read_altitude();
}
