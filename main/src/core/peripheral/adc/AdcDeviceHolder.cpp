...
AdcDeviceHolder::~AdcDeviceHolder()
{
    if (m_continuous_handler) {
        stop_polling();
        CHECK_THROW(adc_continuous_deinit(m_continuous_handler));
    }
    if (m_one_shot_handler) {
        CHECK_THROW(adc_oneshot_del_unit(m_one_shot_handler));
    }
    if (m_cali_handler) {
        CHECK_THROW(adc_cali_delete_scheme_line_fitting(m_cali_handler));
    }
}
...
Device *AdcDeviceHolder::add_device(const std::string &name, AdcMode mode, const std::vector<adc_channel_t> &channels)
{
    ...
    else if (mode == AdcMode::CONTINUOUS) {
    ...
    m_devices[name] =
        std::make_unique<AdcDevice>(std::move(name), mode, CONV_FRAME_SIZE, m_continuous_handler, m_cali_handler);
    return m_devices[name].get();
}
...
{
    try {
        CHECK_THROW(adc_continuous_start(m_continuous_handler));
    }
...
{
    CHECK_THROW(adc_continuous_stop(m_continuous_handler));
}
...
void AdcDeviceHolder::configure_continuous_driver()
{
    ...
    CHECK_THROW(adc_continuous_new_handle(&adc_cfg, &m_continuous_handler));
}
...
void AdcDeviceHolder::configure_one_shot_driver()
{
    ...
    CHECK_THROW(adc_oneshot_new_unit(&adc_one_shot_cfg, &m_one_shot_handler));
}
...
void AdcDeviceHolder::configure_calibration()
{
    ...
    try {
        CHECK_THROW(adc_cali_create_scheme_line_fitting(&cali_config, &m_cali_handler));
    }
    catch (const idf::ESPException &e) {
...
void AdcDeviceHolder::add_device_continuous(const std::vector<adc_channel_t> &channels)
{
    ...
    try {
        CHECK_THROW(adc_continuous_config(m_continuous_handler, &dig_cfg));
    }
    catch (const idf::ESPException &e) {
    }
}

void AdcDeviceHolder::add_device_one_shot(const std::vector<adc_channel_t> &channels)
{
    ...
    for (const auto &chnl : channels) {
        CHECK_THROW(adc_oneshot_config_channel(m_one_shot_handler, chnl, &cfg));
    }
}
